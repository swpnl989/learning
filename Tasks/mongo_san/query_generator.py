...
def get_limit_boolean(limit_boolean):   # It is used to unwind documents that are using arrays
    agg_list = []
    agg = {}
    column_name = "$limit"
    value = int(input("Enter limit to restrict how many documents you want to fetch : "))
...
def get_group_boolean(group_boolean):
    agg_list = []
     
    agg = {}
    dict_name = '$group'
    agg[dict_name] = {}
    column_name = "_id"                                  # in $group _id is Mandatory field
    value = input("Enter the value name to group : ")
    agg[dict_name][column_name] = value
    agg_list.append(agg)    
    return agg_list       
...
def query_generator():
    ...
    if group_boolean == "y":
        agg = get_group_boolean(group_boolean)
        agg_list.extend(agg) 
    print(agg_list,"<------------------->")
    return agg_list
