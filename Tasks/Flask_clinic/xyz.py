...
def get_select(table_name, columns, distinct=False,aggregations=None):
    query1 = []
    ...
    if isinstance(columns,str) and (columns.casefold()=='all' or columns.casefold()=='*'):
        ...
        if distinct:
            ...
            # return a
            return f"SELECT * FROM {table_name.__tablename__}"     # db.select([table_name])
    else:
        if distinct:
            for a in columns:
...
user= ""#["where","groupby"]       
b = {"clinic_id": {"operator": 'in', 'values': ['101', '108', '102']}}
# try:
